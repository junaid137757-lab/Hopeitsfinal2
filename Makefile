...
coverage:
	...
	for f in *.c.gcov; do \
		[ -f "$$f" ] || continue; \
		c=$$(grep -cE '^[[:space:]]*[1-9][0-9]*:[[:space:]]*[0-9]+:' "$$f"); \
		t=$$(grep -cE '^[[:space:]]*[0-9#]+:[[:space:]]*[0-9]+:' "$$f"); \
		covered=$$((covered + c)); \
		total=$$((total + t)); \
	done; \
	pct=$$(awk -v c=$$covered -v t=$$total 'BEGIN{ if(t>0) printf "%.2f", (c/t)*100; else print "0.00" }'); \
	echo "Total: Lines executed:$${pct}% of $$total (summed across all *.c.gcov files, not just the last one)"
