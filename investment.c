...
/* Illustrative reference data for a POC - not real market rates.
   ...
   Stored as parallel arrays (struct-of-arrays) rather than an
   array of structs - see MISRA_DEVIATIONS.md for why. The text
   fields are arrays of `const char *` (pointer to string literal)
   rather than `char[N][WIDTH]` (fixed-width char arrays): a
   fixed-width row wider than its string literal leaves the rest of
   that row implicitly zero-filled, which cppcheck 2.4's MISRA
   addon flags as Rule 9.3 (partial array initialization). A
   pointer array has no such padding - each element's initializer
   (the string literal's address) is a single, complete value. */
static const char * const investmentNames[NUM_INVESTMENT_OPTIONS] =
{
    "Mutual Funds",
    "Gold",
    "Health Insurance",
...
static const char * const investmentExamples[NUM_INVESTMENT_OPTIONS] =
{
...
static const char * const investmentDescriptions[NUM_INVESTMENT_OPTIONS] =
{
...
static const char * const investmentDurations[NUM_INVESTMENT_OPTIONS] =
{
    "3-5 years (long-term)",
    "1-3 years",
...
static const char * const investmentExpectedReturns[NUM_INVESTMENT_OPTIONS] =
{
